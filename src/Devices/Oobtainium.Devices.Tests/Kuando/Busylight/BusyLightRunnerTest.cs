...
using OoBDev.Oobtainium.Devices.Kuando.Busylight;
...
namespace OoBDev.Oobtainium.Devices.Tests.Kuando.Busylight;

[TestClass]
public class BusyLightRunnerTest
{
    ...
    private byte[] GenerateCommands(dynamic[] commands)
    {
        ...
        if (num < nextStep.Length)
        {
            for (i = num; i < nextStep.Length; i++)
            {
                nextStep[i] = 0;
            }
        }
