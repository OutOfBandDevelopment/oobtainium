...
public class GlobalPositioningFixData : INema0183Message
{
    ...
    */
    ...
    public GlobalPositioningFixData(string[] data)
    {
        ...
        if (decimal.TryParse(data.ElementAtOrDefault(2), out var latitude))
            Latitude = latitude;
        LatitudeDirection = data.ElementAtOrDefault(3)[0];

        if (decimal.TryParse(data.ElementAtOrDefault(4), out var longitude))
            Longitude = longitude;
        LongitudeDirection = data.ElementAtOrDefault(5)[0];

        if (int.TryParse(data.ElementAtOrDefault(6), out var fixQuality))
            FixQuality = (FixQuality)fixQuality;
        if (int.TryParse(data.ElementAtOrDefault(7), out var numberOfSatellites))
            NumberOfSatellites = numberOfSatellites;
        if (decimal.TryParse(data.ElementAtOrDefault(8), out var horizontalDilution))
            HorizontalDilution = horizontalDilution;

        if (decimal.TryParse(data.ElementAtOrDefault(9), out var altitude))
            Altitude = altitude;
        AltitudeUnits = data.ElementAtOrDefault(10);

        if (decimal.TryParse(data.ElementAtOrDefault(11), out var height))
            Height = height;
