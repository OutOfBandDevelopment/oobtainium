...
namespace OoBDev.Oobtainium.Retro.Apple2.Dos33;
...
/// </summary>
public readonly struct TrackSectorList(ReadOnlySpan<byte> span)
{

    /// <summary>
    /// $00         Not used
    /// </summary>
    private readonly byte Unused_0 = span[0x00];

    /// <summary>
    /// $01         Track number of next T/S List sector if one was needed or zero if no more T/S List sectors
    /// </summary>
    public readonly byte NextTrack = span[0x01];

    /// <summary>
    /// $02         Sector number of next T/S List sector(if present)
    /// </summary>
    public readonly byte NextSector = span[0x03];

    /// <summary>
    /// $03-$04     Not used
    /// </summary>
    private readonly byte[] Unused_3_to_4 = span.Slice(0x03, 2).ToArray();
    /// <summary>
    ...
    /// </summary>
    public readonly ushort SectorOffset = BitConverter.ToUInt16(span.Slice(0x05, 2));
    /// <summary>
    /// $07-$0B     Not used
    /// </summary>
    private readonly byte[] Unused_7_to_B = span.Slice(0x07, 5).ToArray();
    ...
    /// </summary>
    public readonly TrackSector[] TrackSectorPairs = MemoryMarshal.Cast<byte, TrackSector>(span[0x0c..]).ToArray();
}
