...
namespace OoBDev.Oobtainium.Security;
...
public static class SandboxPath
{
    ...
    /// <returns></returns>
    /// <exception cref="ApplicationException">this will be throw if <c>filePath</c> is not a child of <c>basePath</c>.</exception>
    public static string EnsureSafePath(string basePath, string filePath)
    {
        ...
        if (!composedPath.StartsWith(sandbox))
