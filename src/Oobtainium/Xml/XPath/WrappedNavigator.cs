...
internal class WrappedNavigator(IWrappedNode node) : XPathNavigator
{
    ...
    public override XPathNavigator Clone() => new WrappedNavigator(_node) { _state = _state };
    ...
    public override bool IsSamePosition(XPathNavigator other) => _state switch
    {
        ...
        _ => other switch
        {
            WrappedNavigator wrapped => ReferenceEquals(wrapped._node, _node) && wrapped._state == _state,
            _ => false
        }
    };
